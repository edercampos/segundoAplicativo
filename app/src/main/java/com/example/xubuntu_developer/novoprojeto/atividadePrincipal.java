...
public class atividadePrincipal extends ActionBarActivity {
    ...
    private void atualizaInterface() {
        ...
        while (cursor.moveToNext()) {
            long id = cursor.getLong(cursor.getColumnIndex(contratoDB.Aluno._ID));
    ...
    public class PegaDadosDoServidor extends AsyncTask<Void, Void, String[][]> {
        ...
        protected void onPostExecute(String[][] result) {
            if (result != null) {
                ...
                db.delete(contratoDB.Aluno.NOME_TABELA, null, null);

                for (String linha[] : result) {
                    ContentValues entrada = new ContentValues();
                    entrada.put(contratoDB.Aluno.COLUNA_DATA, linha[0]);
                    entrada.put(contratoDB.Aluno.COLUNA_TIPO, linha[1]);
                    entrada.put(contratoDB.Aluno.COLUNA_MENSAGEM, linha[2]);

                    db.insert(contratoDB.Aluno.NOME_TABELA, null, entrada);
